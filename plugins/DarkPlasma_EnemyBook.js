...
(function () {
  ...
  const unknownData = String(pluginParameters['Unknown Data'] || '??????');
  ...
  const settings = {
    unknownData: String(pluginParameters['Unknown Data'] || '??????'),
    grayOutUnknown: String(pluginParameters['Gray out Unknown']) === 'true',
    maskUnknownDropItem: String(pluginParameters['Mask Unknown Drop Item']) === 'true',
    enemyPercentLabel: String(pluginParameters['Enemy Percent Label'] || 'enemy'),
    dropItemPercentLabel: String(pluginParameters['Drop Item Percent Label'] || 'Drop Item'),
    displayDropRate: String(pluginParameters['Display Drop Rate']) === 'true',
    enableDetailMode: String(pluginParameters['Enable Detail Mode']) === 'true',
    elementIcons: JSON.parse(pluginParameters['Element Icons']).map(icon => Number(icon)),
    weakLabel: String(pluginParameters['Weak Element And State Label'] || 'Weak'),
    resistLabel: String(pluginParameters['Resist Element And State Label'] || 'Resist'),
    devideResistAndNoEffect: String(pluginParameters['Devide Resist And No Effect'] || 'true') === 'true',
    noEffectLabel: String(pluginParameters['No Effect Element And State Label'] || 'No Effect'),
    excludeWeakStates: JSON.parse(pluginParameters['Exclude Weak States'] || '[]').map(state => Number(state)),
    excludeResistStates: JSON.parse(pluginParameters['Exclude Resist States'] || '[]').map(state => Number(state)),
    displayDebuffStatus: String(pluginParameters['Display Debuff Status'] || 'true') === 'true',
    debuffStatusIcons: {
      mhp: {
        small: Number(pluginParameters['Debuff Status Icon MHP Small'] || 48),
        large: Number(pluginParameters['Debuff Status Icon MHP Large'] || 56),
      },
      mmp: {
        small: Number(pluginParameters['Debuff Status Icon MMP Small'] || 49),
        large: Number(pluginParameters['Debuff Status Icon MMP Large'] || 57),
      },
      atk: {
        small: Number(pluginParameters['Debuff Status Icon ATK Small'] || 50),
        large: Number(pluginParameters['Debuff Status Icon ATK Large'] || 58),
      },
      def: {
        small: Number(pluginParameters['Debuff Status Icon DEF Small'] || 51),
        large: Number(pluginParameters['Debuff Status Icon DEF Large'] || 59),
      },
      mat: {
        small: Number(pluginParameters['Debuff Status Icon MAT Small'] || 52),
        large: Number(pluginParameters['Debuff Status Icon MAT Large'] || 60),
      },
      mdf: {
        small: Number(pluginParameters['Debuff Status Icon MDF Small'] || 53),
        large: Number(pluginParameters['Debuff Status Icon MDF Large'] || 61),
      },
      agi: {
        small: Number(pluginParameters['Debuff Status Icon AGI Small'] || 54),
        large: Number(pluginParameters['Debuff Status Icon AGI Large'] || 62),
      },
      luk: {
        small: Number(pluginParameters['Debuff Status Icon LUK Small'] || 55),
        large: Number(pluginParameters['Debuff Status Icon LUK Large'] || 63),
      },
    },
    debuffStatusThreshold: {
      weak: {
        small: Number(pluginParameters['Debuff Status Threshold Weak Small'] || 100),
        large: Number(pluginParameters['Debuff Status Threshold Weak Large'] || 150),
      },
      resist: {
        small: Number(pluginParameters['Debuff Status Threshold Resist Small'] || 100),
        large: Number(pluginParameters['Debuff Status Threshold Resist Large'] || 50),
      },
    },
    verticalLayout: String(pluginParameters['Vertical Layout'] || 'false') === 'true',
    enableInBattle: String(pluginParameters['Enable In Battle'] || 'true') === 'true',
    openKeyInBattle: String(pluginParameters['Open Key In Battle'] || 'pagedown'),
  };
  ...
  class Scene_EnemyBook extends Scene_MenuBase {
    ...
    indexWindowRect() {
      return new Rectangle(
        0,
        this.percentWindowHeight(),
        this.indexWindowWidth(),
        this.indexWindowHeight()
      );
    }
    ...
    indexWindowHeight() {
      return settings.verticalLayout ?
        Graphics.boxHeight - this.percentWindowHeight() :
    ...
    statusWindowRect() {
      ...
      const y = settings.verticalLayout ?
        0 :
        this.indexWindowHeight() + this.percentWindowHeight();
      return new Rectangle(
        x,
        y,
        Graphics.boxWidth - x,
  ...
  class Window_EnemyBookStatus extends Window_Base {
    ...
    drawPageWithVerticalLayout() {
      ...
      this.drawWeakElementsAndStates(0,
        lineHeight * 10 + this.itemPadding(),
        weakAndResistWidth
      );
      this.drawResistElementsAndStates(
        0,
        lineHeight * (11 + this._weakLines) + this.itemPadding(),
        weakAndResistWidth
      );
      ...
      const descX = settings.devideResistAndNoEffect ?
        this.contentsWidth() / 2 + this.itemPadding() / 2 : 0;
      const descWidth = 480;
      if (enemy.meta.desc1) {
        this.drawTextEx(
          enemy.meta.desc1,
          descX,
          this.itemPadding() + lineHeight * 14,
          descWidth
        );
      }
      if (enemy.meta.desc2) {
        this.drawTextEx(
          enemy.meta.desc2,
          descX,
          this.itemPadding() + lineHeight * 15,
          descWidth
        );
      }
    }
    ...
    drawPageWithDetailMode() {
      ...
      const descWidth = 480;
      if (enemy.meta.desc1) {
        this.drawTextEx(enemy.meta.desc1, this.contentsWidth() - descWidth, this.itemPadding() + lineHeight * 10, descWidth);
      }
      if (enemy.meta.desc2) {
        this.drawTextEx(enemy.meta.desc2, this.contentsWidth() - descWidth, this.itemPadding() + lineHeight * 11, descWidth);
      }
    }
    ...
    drawPage() {
      ...
      const descWidth = 480;
      if (enemy.meta.desc1) {
        this.drawTextEx(enemy.meta.desc1, this.contentsWidth() - descWidth, this.itemPadding() + lineHeight * 7, descWidth);
      }
      if (enemy.meta.desc2) {
        this.drawTextEx(enemy.meta.desc2, this.contentsWidth() - descWidth, this.itemPadding() + lineHeight * 8, descWidth);
      }
    }
